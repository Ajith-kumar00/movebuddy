...
const Edit = () => {
    ...
    const fetchMovieData = async () => {
        try {
            ...
            const result = await folderApi.getById(movieId);
            ...
            if (result.success && result.data) {
                ...
                setUserId(movie.userId || "user-123");

                if (movie.image) {
                    setImageBase64(movie.image);
                    setImageSize("Existing image");
                }
            } else {
                setMessage(`Error: ${result.error || 'Failed to fetch movie data'}`);
            }
        } catch (error) {
            setMessage("Network error occurred while fetching movie data");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const result = await folderApi.update(movieId, updateData);

        if (result.success) {
            setMessage(result.message || "Movie updated successfully!");
            ...
            setMessage(`Error: ${result.error || 'Failed to update movie'}`);
    ...
    if (loadingMovie) {
        return (
            <div className="min-h-screen bg-[#092C39] flex items-center justify-center">
                <div className="text-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-white mx-auto mb-4"></div>
                    <p className="text-white text-lg">Loading movie data...</p>
                </div>
            </div>
        );
    }
                        ...
                        <div className="max-w-2xl mx-auto">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                        ...
                                        {imageProcessing ? (
                                            ...
                                            <div className="w-full h-48 flex flex-col items-center justify-center space-y-2">
                                                <img
                                                    ...
                                                    className="max-w-full max-h-full object-contain rounded-lg"
                                                />
                                                <p className="text-gray-300 text-xs">Current image</p>
                                            </div>
                                        ) : (
                                            <>
                                                <svg
                                ...
                                <div className="flex space-x-4 pt-4">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={loading || !title.trim() || !publishingYear}
                                        ...
                                        <div className="flex flex-col items-center space-y-4">
                                            {imageProcessing ? (
                                                ...
                                                <div className="w-full h-80 flex flex-col items-center justify-center space-y-2">
                                                    ...
                                                    />
                                                    {imageSize && (
                                                        <p className="text-gray-300 text-sm">{imageSize}</p>
                                                    )}
                                                </div>
                                            ) : imageBase64 ? (
                                                // Existing Image
                                                <div className="w-full h-80 flex flex-col items-center justify-center space-y-2">
                                                    <img
                                                        ...
                                                        className="max-w-full max-h-full object-contain rounded-lg"
                                                    />
                                                    <p className="text-gray-300 text-sm">Current image</p>
                                                </div>
                                            ) : (
                                                <>
                                                    <svg
                                                    ...
                                                    {/* <p className="text-gray-400 text-sm">Max size: 5MB (will be compressed)</p> */}
                                                </>
                                            )}
                                        </div>
                                    </div>
